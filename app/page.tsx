  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        setError(error instanceof Error ? error.message : 'Ошибка при загрузке'); // Сохраняем сообщение об ошибке
      } finally {
        setLoading(false); // Завершаем загрузку
      }
    };

    fetchUsers(); // Вызываем функцию получения пользователей
  }, []);
  ...
  return (
    <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      ...
      <div className="min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
        {loading && <p>Загрузка пользователей...</p>} {/* Сообщение о загрузке */}
        {error && <p className="text-red-500">{error}</p>} {/* Сообщение об ошибке */}
        {users.length > 0 ? (
